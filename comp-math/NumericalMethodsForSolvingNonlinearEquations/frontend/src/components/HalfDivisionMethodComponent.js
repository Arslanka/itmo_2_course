...
import HalfDivisionMethodService from '../services/HalfDivisionMethodService';
import FunctionPlot from './FunctionPlot';

class HalfDivisionMethodComponent extends Component {
    ...
    handleSubmit = event => {
        ...
        HalfDivisionMethodService.solveHalfDivisionMethodResult(this.state.equation_number, this.state.left, this.state.right, this.state.eps)
            .then((
                    res
                ) => this.setState({
                        solution: res.data
    ...
    render() {
        return (
            <div>
                <form onSubmit={this.handleSubmit}>
                    <label> Equation number:
                        ...
                        <nav><select value={this.state.equation_number.value} onChange={this.handleChangeEquationNumber}
                            ...
                            <input type="text" placeholder="eps" pattern="^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)$"
                ...
                <FunctionPlot f={this.state.f} range={{
                    x: [this.state.left, this.state.right],
                    y: [-10, 10]
                }} />
            </div>
        );
...
export default HalfDivisionMethodComponent;